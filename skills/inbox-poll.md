...
- Call the `outlook` MCP mail-list action, typically `outlook.list_messages`.
...
Without an authorized Outlook MCP connection, the runtime can load the agent definitions, but inbox action calls will fail clearly. Use `sample-data/inbox.json` only as a shape reference for the messages the Outlook MCP returns in production.
