...
|---|---|---|
| Agent logic | LLM reasons from `.agent.md` + skills text, no `tools/` | Same, plus one `tools/match_rule.py` classifier |
| `tools/` directory | ❌ none (by design) | ✅ one Python tool (`match_rule.py`, deterministic classification) |
| I/O path | MCP only (Outlook & Teams managed connectors); DRY RUN renders deliverables as text when settings are placeholders | Same |
...
| `function_app.py` | One line: `app = create_function_app()` | Identical one line |
| Hand-written Python | ~1 line | ~1 line + `tools/match_rule.py` |
