...
**Goal:** verify the agent recognizes VIP urgency and routes to Teams. In offline mode, verify the local log; with connectors authorized, verify the real Teams post.
...
- No `office365_*` / `teams_*` calls are made. The run ends `Triaged 5: 2 escalate, 2 reply, 1 summarize`.
